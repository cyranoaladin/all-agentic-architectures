...
export default function A08GraphMemory() {
  ...
  const [out, setOut] = useState<{ answer?: string; graph?: unknown; }>({});
  const [loading, setLoading] = useState(false);
  const run = async () => {
    setLoading(true);
    try {
      const r: { answer?: string; graph?: unknown; } = await fetch("/api/a08/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, question }) }).then(r => r.json());
      setOut(r);
  ...
  return (<section><h2>A08 — Graph Memory</h2>
    ...
    {Boolean(out.graph) && <div className="result"><h4>Relations</h4><pre>{JSON.stringify(out.graph, null, 2)}</pre></div>}
  </section>);
}
