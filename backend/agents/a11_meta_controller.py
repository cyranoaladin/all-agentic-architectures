...
def run(query: str) -> Out:
    q = query.lower()
    if any(k in q for k in ["cherche","news","wwdc","en temps réel","aujourd'hui","dernières"]):
        tool = web_search_tool(3)
        hits = tool.run(query)
        llm = make_llm()
        ans = llm.invoke(f"Question: {query}\nRésultats web: {hits}\nSynthèse brève.").content
